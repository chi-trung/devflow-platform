...
/**
 * "The Intelligence" section — copy column describing weighted wiki recall +
 * a plan panel mockup shaped like AiPlanPanel output: the planner drafts
 * steps grounded in wiki entries, each cited as [n] ID with its real status
 * badge and `w {weight}` weight chip (the same weight display as
 * KnowledgeEntryCard). No terminal exists in the product, so there is no
 * terminal chrome here.
 */
    ...
    <section className="border-b border-border py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          {/* ── Plan panel mockup (AiPlanPanel output shape) ── */}
          <BrowserFrame>
            {/* panel header */}
            <div className="mb-3 flex items-center justify-between gap-2">
              <span className="inline-flex min-w-0 items-center gap-1.5 text-xs font-semibold text-violet-400">
                <Brain className="size-3.5 shrink-0" aria-hidden />
                <span className="truncate">{t("landing.intel.terminalPrompt")}</span>
              </span>
            ...
            <p className="mb-3 rounded-lg border border-border/60 bg-card p-2 text-[12px] leading-relaxed text-foreground">
              {t("landing.intel.terminalQuery")}
            </p>

            {/* plan shape tag */}
            <div className="mb-3 flex items-center justify-between">
              <span className="inline-flex rounded-md border border-border bg-elevated px-2 py-0.5 font-mono text-[10.5px] uppercase tracking-wider text-muted-foreground">
                {t("landing.intel.terminalAgentTag")}
              </span>
            </div>

            {/* grounded wiki citations */}
